...
from ..models import Product, Comment  # 모델 정의 (질문, 답변, 댓글)

# =======================================
# 질문에 대한 댓글 작성 뷰 (로그인 필요)
# =======================================
@login_required(login_url='common:login')  # 로그인이 필요하며, 로그인하지 않으면 로그인 페이지로 리다이렉트
def comment_create_product(request, product_id):
    """ sales 질문에 댓글 작성 """
    ...
    if request.method == "POST":
        ...
        if form.is_valid():  # 폼 데이터가 유효할 경우
            ...
            comment.Product = product  # 댓글이 달린 질문을 설정
            comment.save()  # 댓글을 DB에 저장
        return redirect('sales:detail', product_id=product.id)  # 댓글 작성 후 질문 상세 페이지로 리다이렉트
    else:
        # GET 요청인 경우 빈 폼을 생성하여 댓글 작성 페이지를 보여줌
        form = CommentForm()

    # 작성된 댓글과 대댓글을 조회하여 템플릿으로 전달
    comments = product.comments.filter(parent__isnull=True).order_by('create_date').prefetch_related('replies')
    ...
    return render(request, 'sales/comment_form.html', context)  # 댓글 작성 페이지를 렌더링

# =======================================
# 질문 댓글 수정 뷰 (로그인 필요)
# =======================================
@login_required(login_url='common:login')  # 로그인이 필요하며, 로그인하지 않으면 로그인 페이지로 리다이렉트
def comment_modify_product(request, comment_id):
    """ sales 질문 댓글 수정 """
    ...
    if request.user != comment.author:
        ...
        return redirect('sales:detail', product_id=comment.product.id)  # 권한이 없으면 질문 상세 페이지로 리다이렉트
    ...
    if request.method == "POST":
        ...
        if form.is_valid():  # 폼 데이터가 유효할 경우
            ...
            return redirect('sales:detail', product_id=comment.product.id)  # 수정 후 질문 상세 페이지로 리다이렉트
    else:
    ...
    # 템플릿에 전달할 데이터 설정
    context = {'form': form}
    return render(request, 'sales/comment_form.html', context)  # 댓글 수정 페이지를 렌더링

# =======================================
# 질문 댓글 삭제 뷰 (로그인 필요)
# =======================================
@login_required(login_url='common:login')  # 로그인이 필요하며, 로그인하지 않으면 로그인 페이지로 리다이렉트
def comment_delete_product(request, comment_id):
    """ sales 질문 댓글 삭제 """
    ...
    if request.user != comment.author:
        ...
        return redirect('sales:detail', product_id=comment.product.id)  # 댓글 삭제 후 질문 상세 페이지로 리다이렉트
