...
# from sales.ai_system.ai_pybo import start_ai
# from pybo.ai_system.ai_pybo import start_ai
from ..forms import ProductForm
from ..models import Product, Answer
...
def product_modify(request, product_id):
    ...
    if request.user != product.author:
        ...
        return redirect('pybo:detail', product_id=product.id)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return JsonResponse({'redirect_url': reverse('pybo:detail', args=[product.id])})
    else:
    ...
    # 템플릿에 폼을 전달하여 렌더링
    context = {'form': form}
    return render(request, 'pybo/product_form.html', context)
...
def product_vote(request, product_id):
    ...
    if request.user == product.author:
        ...
        product.voter.add(request.user)  # 추천 처리
